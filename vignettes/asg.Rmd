---
title: Tutorial on the asg package - Network construction using the St. Nicolas House Analysism
shorttitle: "asg package tutorial 0. 10.1"
...
vignette: >
  ...
  %\VignetteIndexEntry{Tutorial on the asg package - Network construction using the St. Nicolas House Analysis}
...
```{r fig.width=9,fig.height=6,fig.cap="Birth weight data variable interactions"}
...
```{r label=dec,fig.width=8,fig.height=5,fig.cap="Decathlon Data 1988"}
par(mai=c(0.2,0.2,0.7,0.1))
data(decathlon88)
head(decathlon88)
A=asg.new(decathlon88,method="spearman",alpha=0.1)
cols=rep("salmon",10)
cols[names(A$data) %in% c("jave","shot","disc","pole")]="skyblue"
plot(A,layout="sam",vertex.color=cols,vertex.size=7,cex=1.1,edge.width=5)
asg.rsquare(A)
...
```{r label=start,fig.width=5,fig.height=1.5,fig.cap="An association chain"}
...
```{r label=plot,fig.width=8,fig.height=4,fig.cap="Swiss data associations"}
...
The  package  as well  allows you to create  your some types of regular or random
graphs using the _mgraph.new_ function. Here an example:

```{r,fig=TRUE,fig.height=6,fig.height=4}
B = mgraph.new(type="angie",nodes=15,edges=20)
plot(B,layout="sam")
```

You can as well highlight  central nodes with red colors and isolated notes in
blue color, using the so called harmonic centrality. Here an example:

```{r,fig=TRUE,fig.width=8,fig.height=5}
par(mar = c(5, 1, 1, 1), xpd = TRUE)
B=mgraph.new(type="barabasi",nodes=15,edges=20)
h=mgraph.harmonic_centrality(B)
h
cols=mgraph.nodeColors(B,type="harmonic")
plot(B,vertex.size=4+5*h,layout="sam",vertex.color=cols)
lg=mgraph.nodeColors(B,type="harmonic",legend=TRUE)
legend("top",fill=lg$col, inset=c(1.0,1.0),legend=lg$legend,ncol=6,)
```
